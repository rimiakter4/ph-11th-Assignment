...
const Register = () => {
  ...
  return (
    /* ১. ডার্ক মোড ব্যাকগ্রাউন্ড অ্যাডজাস্টমেন্ট */
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-sky-100 via-indigo-100 to-purple-100 dark:from-gray-900 dark:via-indigo-950 dark:to-gray-950 px-4 py-10 transition-colors duration-500">
      <motion.div
        initial={{ opacity: 0, y: 40 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.7 }}
        /* ২. ডার্ক মোড গ্লাস-মরফিজম কার্ড */
        className="backdrop-blur-xl bg-white/30 dark:bg-black/40 p-8 rounded-3xl shadow-2xl w-full max-w-2xl border border-white/40 dark:border-white/10"
      >
        <h2 className="text-4xl font-extrabold text-center text-indigo-700 dark:text-indigo-400 mb-6">Register</h2>

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="font-semibold text-gray-700 dark:text-gray-300">Full Name</label>
              <input type="text" {...register("name", { required: "Name is required" })} className="w-full mt-1 px-4 py-3 rounded-xl bg-white/80 dark:bg-gray-800/80 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 transition-all" placeholder="Enter your name" />
              {errors.name && <p className="text-red-600 dark:text-red-400 text-sm mt-1">{errors.name.message}</p>}
            </div>
            <div>
              <label className="font-semibold text-gray-700 dark:text-gray-300">Photo URL</label>
              <input type="text" {...register("photo", { required: "Photo URL is required" })} className="w-full mt-1 px-4 py-3 rounded-xl bg-white/80 dark:bg-gray-800/80 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 transition-all" placeholder="Enter photo link" />
              {errors.photo && <p className="text-red-600 dark:text-red-400 text-sm mt-1">{errors.photo.message}</p>}
            </div>
          </div>

          <div>
            <label className="font-semibold text-gray-700 dark:text-gray-300">Email Address</label>
            <input type="email" {...register("email", { required: "Email is required" })} className="w-full mt-1 px-4 py-3 rounded-xl bg-white/80 dark:bg-gray-800/80 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 transition-all" placeholder="Enter your email" />
          </div>

          <div>
            <label className="font-semibold text-gray-700 dark:text-gray-300">Select Role</label>
            <select {...register("role", { required: "Please select a role" })} className="w-full mt-1 px-4 py-3 rounded-xl bg-white/80 dark:bg-gray-800/80 border border-gray-300 dark:border-gray-700 outline-none font-medium text-gray-600 dark:text-gray-300 transition-all">
              <option value="">Choose Role</option>
              <option value="buyer">Buyer</option>
              <option value="manager">Manager</option>
            </select>
          ...
          <div>
            ...
            <div className="relative mt-1">
              <input type={showPassword ? "text" : "password"} {...register("password", { required: "Password is required", minLength: { value: 6, message: "Min 6 chars" } })} className="w-full px-4 py-3 rounded-xl bg-white/80 dark:bg-gray-800/80 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 transition-all" placeholder="••••••••" />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-gray-600 dark:text-gray-400" onClick={() => setShowPassword(!showPassword)}>
                {showPassword ? <FaEyeSlash size={20} /> : <FaEye size={20} />}
              </span>
            </div>
          ...
          <button type="submit" className="w-full bg-indigo-600 dark:bg-indigo-500 text-white py-4 rounded-xl font-bold hover:bg-indigo-700 dark:hover:bg-indigo-600 transition shadow-lg active:scale-95">Create Account</button>
        </form>

        <div className="text-center my-4 text-gray-600 dark:text-gray-400 font-semibold italic flex items-center gap-2 before:content-[''] before:flex-1 before:border-b dark:before:border-gray-700 after:content-[''] after:flex-1 after:border-b dark:after:border-gray-700">OR</div>
        
        <button onClick={handleGoogle} className="w-full flex items-center justify-center gap-3 bg-white dark:bg-gray-800 text-gray-800 dark:text-white border border-gray-300 dark:border-gray-700 py-3 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition font-semibold shadow-sm">
          <FaGoogle className="text-xl text-red-500" /> Register with Google
        </button>

        <p className="mt-6 text-center text-gray-700 dark:text-gray-300">
          Already have an account?{" "}
          <Link to="/login" className="text-indigo-600 dark:text-indigo-400 font-bold hover:underline">Login</Link>
        </p>
      </motion.div>
    </div>
  );
